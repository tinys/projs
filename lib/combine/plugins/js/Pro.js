...
(function(module){
  ...
  function combine(fileMap,Opts,beCombineMap,key){
    ...
    return codeStr.replace(regRequireTemp,function(){
      ...
      if (key) {
        ...
        Opts.file.path = uri;
        return (beCombineMap[uri] == 1 || filePathCache[indexKey]['beCombineMap'][uri] == 1) ? '' : combine(fileMap,Opts,beCombineMap,key)+'\n';
      }
    });
  }
  ...
  module.exports = {
    combine: function(fileMap,Opts,beCombineMap,callback){
      ...
      callback(combine(fileMap,Opts,beCombineMap));
    },
    params: {
      "lib" : {
        name: "包路径"

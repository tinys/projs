...
(function(module){
  ...
  function getKeyPath(key,Opts){
    uri = path.normalize(Opts.baseDir+'/pro_modules/'+key+'/index.js');
  ...
  function combine(fileMap,Opts,beCombineMap){
    ...
    // 如果没有define时，添加一个
    if(!isStrict){
      codeStr = '\ndefine (function(module){'+codeStr+'});\n';
    }

    codeStr = codeStr.replace(/define\s*\(/gi, '\ndefine(' + getKey(filePath.replace(baseDir, ''),Opts) + ',');
      ...
      matchArr.forEach(function(key) {
        ...
        if (importKey = util.getImportValue(key)) {
          ...
          if(isStrKey){
            ...
            relativePath = util.getRelativePathByKey(importKey,filePath,baseDir);
          }
    ...
    return codeStr.replace(regRequireTemp,function(){
      ...
      if (key) {
        ...
        if(isStrKey){
          ...
          util.getAbsolutePathByKey(key,filePath);
        }

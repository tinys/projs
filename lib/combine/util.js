...
(function(module) {
  ...
  function getFileCodeStr(filePath) {
    var codeStr = '';
  ...
  function replaceImgPathToRelativePath(source,filePath){
    ...
    if (arrImageUrl){
      arrImageUrl.forEach(function(uri){
        var imgPath = getImportValue(uri);
        _replaceImgHash[imgPath] = getRelativeByImgPath(filePath,imgPath)
      });

      Object.keys(_replaceImgHash).forEach(function(key){
        var reg = new RegExp(key.replace(/(\"|\'|\(|\)|\.|\?|\=)/gi,function(a){
          return '\\'+a;
        }),'ig');
   ...
   * 根据图片地址获取其相对路径
   *
   * @method getRelativeByImgPath
   *
   * @param  {[type]}             topPath      [description]
   * @param  {[type]}             importedPath [description]
   * @param  {[type]}             imgPath      [description]
   *
   * @return {[type]}
   */
  function getRelativeByImgPath(importedPath,imgPath){
    return path.relative(Opts.filePathUri,path.resolve(importedPath,'..',imgPath)).replace('../','');
  }

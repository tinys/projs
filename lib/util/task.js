...
(function(module) {
	...
	var temp = path.normalize(__dirname+'../../../temp/');
	/**
	 * 压缩
	...
	function compress(from, to, config, isEmpty) {
					...
					try{
						...
						combine({
							...
							if(error){
								...
								uri = uri.replace(to,originTo);
								uriPath = path.dirname(uri);
			...
			if(errorMsg.length){
				...
				execute('chmod',['777',to,'-R'],{
					onComplete: function(){
						...
						if(_otherDir.length){
							var arr = [];
							if(config.js_compress_path_clear){
								arr.push('js');
							}
							if(config.css_compress_path_clear){
								arr.push('css');
							}
		...
		if(from){
			...
			removeFolder(temp,{
				onComplete: function(){
					execute('cp', ['-rf',originTo, to], {
						onComplete: function(){
							removeFolder(originTo,{
								onComplete: _compress
							});
						}
					});

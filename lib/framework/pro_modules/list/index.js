...
define(function(module){
	 ...
	 */
	module.exports = function(box,Opts){
		var that = {},
			init,
			conf,
			nodeListCache = [],
		...
		function insert(Opts){
			...
			// 创建新节点
			newNodeList = that.create(_conf.source,_conf.template);
			...
			if(newCount){
				...
				// 如果有目标结点
				if(target){
					$(target)[nodeOperation[pos]](util.merge(newNodeList));
				}else{
					var _box = that.getNodeListBox() || box;
					that.removeNodeList();
					$(_box).append(util.merge(newNodeList));
				}
			}
		...
		function createNodeList(source,template){
			...
			re = $ET(template,source).toString();
			re = $builder(re).list[conf.nodeType] || [];
